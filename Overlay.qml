...
FocusScope {
    ...
    // Target Omarchy's user bindings Lua file
    ...
    // Reload Hyprland (Re-evaluates Lua configs)
    Process {
        id: reloadProcess
    ...
    function parseConfig(text) {
        ...
        for (let line of lines) {
            line = line.trim()
            if (line === "-- BEGIN MANAGED KEYBINDS PLUGIN") { inBlock = true; continue }
            if (line === "-- END MANAGED KEYBINDS PLUGIN") { inBlock = false; continue }
            
            if (inBlock) {
                // Match Omarchy bind syntax: o.bind("MOD + KEY", "Desc", "Command")
                ...
                // Match Omarchy unbind syntax: hl.unbind("MOD + KEY")
    ...
    function saveConfig() {
        let blockStart = "-- BEGIN MANAGED KEYBINDS PLUGIN"
        let blockEnd = "-- END MANAGED KEYBINDS PLUGIN"
        
        let content = "\n" + blockStart + "\n"
        for (let bind of keybinds) {
            if (bind.type === "bind") {
                let cmd = bind.command.replace(/"/g, '\\"')
                let desc = bind.desc.replace(/"/g, '\\"')
                content += `o.bind("${bind.keys}", "${desc}", "${cmd}")\n`
        ...
        // Shell script to merge safely, preserving user's manual o.bind overrides
        let mergeScript = `
            FILE="${configPath}"
            TEMP="/tmp/plugin_keybinds_merge"
            touch "$FILE"
    ...
    function editBinding(index) {
        ...
        if (bind.type === "unbind") {
            ...
            else actionCombo.currentIndex = 1 // Custom
    ...
    function saveBinding() {
        ...
        if (capturedKey === "UNKNOWN" || fullKeys.trim() === "") {
            console.error("Invalid key captured.")
            return
        }
