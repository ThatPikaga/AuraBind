...
function parseManagedBlock(body) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (result.length > 5000) break; // Cap record limit
  }
  return result;
}
...
function parseLuaSourceForBindings(text, sk) {
  ...
  if (text.length > 2000000) text = text.substring(0, 2000000); // Cap input size
...
function parseCalls(source, sk) {
  ...
  while ((match = re.exec(source)) !== null) {
    if (results.length > 5000) break; // Cap record limit
    ...
    if (desc.length > 200) desc = desc.substring(0, 200); // Cap field length
...
function renderManagedBody(defaults, userBindings) {
  ...
  // Iterate backwards to keep the most recent definition of a key
